...
4. _Ziel:_ Filtere nach `http`. Finde das Paket mit dem `GET /index.php` Request und mache einen Screenshot bzw. Log-Auszug.
...
2. Starte den Capture in einer Shell:

   ```bash
   tshark -i eth0 -w /captures/aufgabe3.pcap
   ```

3. Starte eine andere Shell und lade die Datei per FTP auf Maschine A hoch (Login: `student`, Passwort: `secret`):
...
4. Beende tshark.
5. _Ziel:_ Öffne das Pcap in Wireshark. Suche nach dem **Passwort** im Klartext (Filter: `ftp`) und dem **Inhalt der Textdatei** (Filter: `ftp-data`). Mache Screenshots bzw. Log-Auszüge.
...
3. _Ziel:_ Analysiere den Mitschnitt. Du wirst den Inhalt der Webseite _nicht_ lesen können. Identifiziere stattdessen den **TLS Handshake** (Client Hello, Server Hello, Certificate Exchange). Mache Screenshots bzw. Log-Auszüge der relevanten Pakete.
